...
for i in 0..7
  User.create(email: emails[i], password: Faker::Internet.password(min_length: 8),
...
for i in 0..7
  Event.create(start_date: Faker::Date.forward(days: 100), duration: durations, title: Faker::Book.title,
               ...
               location: Faker::Address.city, admin: User.all.sample)
end


for i in 0..7
  Attendance.create(stripe_customer_id: Faker::Number.number,
                    participant: User.all.sample, event: Event.all.sample)
end
